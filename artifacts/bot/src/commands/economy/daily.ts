...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (user.lastDaily) {
      const diff = now - new Date(user.lastDaily).getTime();
    ...
    const amount = cfg.dailyAmount ?? 500;
    ...
    await logTransaction(interaction.guildId!, null, interaction.user.id, amount, "daily", "Recompensa diaria");

    await interaction.reply({
      embeds: [successEmbed("Recompensa Diaria", `¡Reclamaste tu recompensa diaria de **${formatCurrency(amount)}**! 💰`)],
    });
  },
};
